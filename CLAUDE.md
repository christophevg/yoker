...
│   │                        #   inject_tools(), register_primary_agent(), get_backend();
...
- `Agent` no longer carries a `_agent_id` attribute (Session owns the registry key). The Session stamps the session-assigned id on the Agent as `agent._session_id` (Session-managed metadata, declared on `Agent` as `_session_id: str | None = None`) in both `_spawn_internal` and `register_primary_agent` — the bridge `Session.send` uses to resolve `Agent` instances back to ids for event payloads. `Session.release(agent)` removes by identity (single cleanup path; no `_agent_id_for` / `_release` helpers). `Session.agent` read-only property (was `primary_agent`) backed by `register_primary_agent()`, which also overrides the agent's backend with the session-shared one.
