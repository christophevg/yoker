...
- Tool framework redesigned: tools are plain functions or callable classes, guardrail metadata comes from `yoker.tools.annotations` markers, and `ToolRegistry` stores `ToolSpec` objects built via `yoker.tools.tool_spec.build_tool_spec()`.
