...
│   ├── __init__.py          # Public re-exports + inlined agent() factory
│   ├── _internal.py         # build_agent(), run_sync(), thinking mapping
│   ├── one_shot.py          # Layer 1: process() one-shot (no ask/complete/run_skill)
│   └── session.py           # Layer 3: session() facade; spawn() returns Agent (no prompt)
...
│   ├── __init__.py          # Exports Session, Message (SpawnResult is internal)
...
│   │                        #   spawn() -> Agent, _spawn_and_run() -> SpawnResult (internal),
│   │                        #   send(), inject_tools(), register_primary_agent(), get_backend(),
│   │                        #   _release() cleanup; loads plugin agent defs via
│   │                        #   register_configured_plugin_agents()
│   ├── spawn_result.py      # SpawnResult(agent_id, response) — internal to agent tool
...
- MBI-003 Python API redesign complete: `src/yoker/agent/` renamed to `src/yoker/core/`; Agent is fully Session-agnostic (no `session` param, no `_session` attribute, no `Agent.spawn()`); the `yoker.agent` name is now the factory function in `yoker.api`.
- Public Python API surface (in `yoker/__init__.py`): `process`, `run_sync`, `agent`, `session`, `Agent`, `Session`, `ApiSession`, `ThinkingLiteral`. Removed: `ask`, `ask_sync`, `complete`, `complete_sync`, `run_skill`, `run_skill_sync`, `build_agent`.
- Three-layer facade: Layer 1 `yoker.process(prompt, **kwargs)` (one-shot, `tools=[]`, `system_prompt=""`); Layer 2 `yoker.agent(**kwargs) -> Agent` (reusable; call `agent.process(prompt)` or `agent.do(skill_name, prompt, args="")`); Layer 3 `yoker.session(id=...) -> ApiSession` (multi-turn; use `session.agent.process(...)`).
- Single sync helper: `yoker.run_sync(coro)` wraps `asyncio.run` — dropped per-function `*_sync` wrappers.
- Session.spawn(name) returns a reusable Agent (no prompt, no response, no SpawnResult in public API). The internal `Session._spawn_and_run(name, prompt, requester=, timeout_seconds=)` is used by the `agent` tool for run-to-completion semantics; `_release(agent_id)` cleans up.
