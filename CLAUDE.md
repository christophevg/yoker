...
│   ├── diff.py              # generate_diff() — shared unified-diff helper (update tool + approval flow)
...
│   │   └── path.py          # PathGuardrail (traversal, size, extension, protected_files checks)
...
- `SpawnResult` removed entirely: the `agent` tool folds `Session._spawn_internal(name, requester=)` + `child.process(prompt)` (with `asyncio.wait_for` timeout) + `await session.release(child)` inline, and renders `agent_id: <id>\n\n<response>` (no `_render_spawn_result` helper, no `_spawn_and_run` method).
...
- MBI-009 T12 `protected_files` guardrail complete (Option A: simple block + interactive approve-on-diff). `PermissionsConfig.protected_files: tuple[str, ...]` holds an fnmatch glob denylist (default: `Makefile`, `pyproject.toml`, `yoker.toml`, `.git/config`, `.git/hooks/*`, `.github/workflows/*.yml`, `uv.lock`, `poetry.lock`, …). `PathGuardrail._check_protected_files(resolved)` matches the relative path from the allowed root AND the basename via `fnmatch.fnmatchcase`, called from the `write` and `update` branches of `validate()` (before extension checks); `read` is never blocked. An empty tuple disables all protections (explicit opt-out). SOFT guardrail — protects against powerful mistakes, not malicious agents. Public `PathGuardrail.is_protected(path) -> bool` is the entry point used by the processing loop. Interactive flow (Option D, in `_run_tool`): when `agent._approval_handler` is wired (set by `cli/chat.py` for interactive sessions via `_wire_approval_handler`), `PathGuardrail.interactive_approvals = True` skips the simple block and `_maybe_block_protected` in `yoker/core/_processing.py` generates a unified diff (`yoker/tools/diff.py::generate_diff`, also used by `yoker/builtin/update.py`) and awaits `agent._approval_handler(path, diff)`; approval falls through to `_execute_tool`, denial returns a blocked `ToolResult`. `BatchUIHandler.confirm_approval` always returns False (simple block fires first in batch mode). `InteractiveUIHandler.confirm_approval` renders the diff via `_show_diff_content` and prompts y/N (empty/EOF/Ctrl+C = deny, fail-safe). Agent stays UI-agnostic — `_approval_handler` typed as `Callable[[str, str], Awaitable[bool]] | None`. `examples/custom_handler.py::PrintUIHandler` updated to implement the new protocol method.
