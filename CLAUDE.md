...
│                           #   Agent, Session, ApiSession, ThinkingLiteral
...
│   ├── _plugins.py          # Plugin loading helpers (session=None for standalone Agent)
...
├── api/                     # Thin Pythonic API (MBI-003, single-module facade)
│   └── __init__.py          # process(), do(), agent(), session(), run_sync(),
│                            #   ThinkingLiteral, private helpers (_thinking_mode,
│                            #   _apply_model_provider, _build_agent_definition,
│                            #   _filter_tools, _filter_skills, _session_config,
│                            #   _resolve_primary_definition)
...
│   │                        #   spawn() -> Agent, _spawn_and_run() -> (agent_id, response) tuple,
│   │                        #   _agent_id_for(agent) registry-lookup helper, release(agent) public,
│   │                        #   primary_agent read-only property, send(), inject_tools(),
│   │                        #   register_primary_agent(), get_backend(), _release() cleanup;
...
- MBI-003 Python API reduced to a thin single-module facade (`yoker/api/__init__.py`): `yoker.agent` is the factory function (builder → customised Config → Agent); `yoker.process` / `yoker.do` are one-shot helpers built on `yoker.agent`; `yoker.session` is an async context manager yielding the real `yoker.session.Session` (no facade class). `yoker/api/_internal.py`, `yoker/api/one_shot.py`, `yoker/api/session.py`, and the `ApiSession` facade class removed.
- Public Python API surface (in `yoker/__init__.py`): `Agent`, `Session`, `Config`, `process`, `do`, `agent`, `session`, `run_sync` (plus `ThinkingLiteral` and the existing context/event/exception exports). Removed: `ApiSession`, `ask`, `ask_sync`, `complete`, `complete_sync`, `run_skill`, `run_skill_sync`, `build_agent`, `SpawnResult`.
- Thin wrappers: `yoker.process(prompt, **kwargs)`, `yoker.do(skill_name, prompt, args="", **kwargs)`, `yoker.agent(**kwargs) -> Agent` (reusable; call `agent.process(prompt)` or `agent.do(skill_name, prompt, args="")`), `yoker.session(id=..., *, persist=True, fresh=False, **kwargs) -> AsyncContextManager[Session]` (yields the real Session; use `session.primary_agent.process(...)`).
- Single sync helper: `yoker.run_sync(coro)` wraps `asyncio.run` — dropped per-function `*_sync` wrappers.
- `SpawnResult` removed entirely: `Session._spawn_and_run` now returns a `(agent_id, response)` tuple; the `agent` tool renders `agent_id: <id>\n\n<response>` inline (no `_render_spawn_result` helper).
- `Agent` no longer carries a `_agent_id` attribute (Session owns the registry key). `Session._agent_id_for(agent)` does the registry lookup; `Session.release(agent)` is the public cleanup wrapper over `_release(agent_id)`. `Session.primary_agent` read-only property backed by `register_primary_agent()`.
...
- `Agent` is a single unified class in `yoker/core/__init__.py` backed by private helper modules (`_setup.py`, `_processing.py`, `_plugins.py`, `_tools.py`).
