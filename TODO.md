...
### Phase 7: Quart Webapp (Chat Frontend)

**Overview**: Web-based chat interface for Yoker with magic-link authentication and Ollama API key management. Provides a user-friendly alternative to the CLI/TUI interface.

**Dependencies**: Phase 3 (Backend Integration) and Phase 4 (Agent Runner) must be completed first. Context management research (task 3.3) is critical for per-user session isolation.

- [x] **7.1 Quart Framework Setup** (2026-05-06)
  - Initialize Quart application with async support
  - Set up project structure for webapp (src/yoker/webapp/)
  - Configure Quart with Yoker's configuration system
  - Implement WebSocket support for real-time streaming
  - Define complete WebSocket protocol specification (all event types, JSON schemas)
  - Implement WebSocketEventHandler for event bridging (uses existing serialize_event)
  - Create basic routing structure:
    - GET /health - Health check (public, no auth)
    - WebSocket /ws/chat - Real-time chat stream
  - Add CORS configuration for frontend integration
  - Implement error handling strategy (ErrorEvent via WebSocket, graceful disconnect)
  - Document session lifecycle model (one Agent per WebSocket connection)
  - Implement context isolation per WebSocket session (session-specific storage path)
  - Add ToolContentEvent deserialization support (missing in events/recorder.py)
  - Write unit tests for application factory, WebSocket handler, health endpoint
  - **Security**: CSWSH prevention (origin validation), session limits, message validation
  - **Entry points**: `uv run uvicorn yoker.webapp:app` and `uv run python -m yoker.webapp`
  - **See**: `reporting/7.1-quart-framework/summary.md`
  - Write integration tests for full turn lifecycle, tool call streaming
  - **Acceptance**: `uv run python -m yoker.webapp` starts webapp on localhost:5000
  - **Acceptance**: WebSocket connects and receives events from Agent
  - **Acceptance**: Health endpoint returns {"status": "healthy", "version": "..."}
  - **Priority**: P1 (foundational for webapp)
  - **Depends on**: Phase 3, Phase 4
  - **Design Doc**: `analysis/api-quart-webapp.md`
  - **API Review**: `analysis/api-quart-webapp-review.md`

- [ ] **7.2 SMTP Configuration and Validation**
  - Add SMTP configuration to config schema (smtp_host, smtp_port, smtp_user, smtp_pass)
  - Add EMAIL_ALLOWED configuration (comma-separated whitelist)
  - Implement SMTP connection validation on startup
  - Add configuration validation error messages
  - Add tests for SMTP configuration validation
  - Document SMTP configuration in configuration guide
  - **Acceptance**: Invalid SMTP config fails fast with clear error; valid config passes validation
  - **Priority**: P1 (required for authentication)
  - **Depends on**: 7.1

- [ ] **7.3 Magic Link Email Authentication System**
  - Design authentication flow: request → email → magic link → session
  - Implement secure token generation (cryptographically random, time-limited)
  - Create magic link token storage (in-memory with expiration, or Redis for production)
  - Implement email sending with SMTP
  - Create /auth/request endpoint (POST email address)
  - Create /auth/verify/:token endpoint (GET with token validation)
  - Implement email whitelist validation (EMAIL_ALLOWED check)
  - Add rate limiting for auth requests (prevent abuse)
  - Write unit tests for auth flow (mock SMTP)
  - **Acceptance**: Magic link sent to whitelisted email; token expires after configurable time; non-whitelisted email rejected
  - **Priority**: P1 (required for user access)
  - **Depends on**: 7.2

- [ ] **7.4 Session Management and API Key Storage**
  - Design session token storage (JWT or server-side with secret)
  - Create session cookie/token management
  - Implement Ollama API key collection via magic link page
  - Create API key storage endpoint (POST to store key with session)
  - Store API key in browser localStorage (client-side)
  - Send API key to backend with each request (Authorization header or cookie)
  - Associate API key with user session in backend
  - Implement session expiration and renewal
  - Write unit tests for session management
  - **Acceptance**: User can authenticate via magic link; Ollama API key stored securely; session persists across page reloads
  - **Priority**: P1 (required for user sessions)
  - **Depends on**: 7.3

- [ ] **7.5 Frontend: Chat Interface Design**
  - Design responsive chat UI layout (sidebar for history, main chat area)
  - Implement modern CSS styling (minimal, clean design)
  - Create input box with multiline support (textarea, not single-line input)
  - Implement conversation history display (scrollable, clear role differentiation)
  - Add loading indicators and animations
  - Implement responsive design for mobile devices
  - Add accessibility features (ARIA labels, keyboard navigation)
  - **Acceptance**: UI renders correctly on desktop and mobile; input supports multiline; history is readable and scrollable
  - **Priority**: P1 (core frontend requirement)
  - **Depends on**: None (can start in parallel with backend work)

- [ ] **7.6 Frontend: WebSocket Integration and Real-time Streaming**
  - Implement WebSocket client connection in JavaScript
  - Handle connection lifecycle (connect, disconnect, reconnect)
  - Implement real-time message streaming (tokens appear as they're generated)
  - Display thinking sections (gray/muted styling, collapsible)
  - Display tool calls (formatted, with result display)
  - Handle error states (connection lost, API errors)
  - Implement stop generation button
  - Write integration tests (mock WebSocket server)
  - **Acceptance**: Messages stream in real-time; thinking/tool calls display correctly; user can stop generation
  - **Priority**: P1 (core user experience)
  - **Depends on**: 7.5, 7.1

- [ ] **7.7 Backend: Yoker Agent Integration**
  - Create WebSocket handler for chat messages
  - Integrate Yoker Agent class with WebSocket streaming
  - Implement per-user context isolation (one context per authenticated user)
  - Map user session to Ollama API key (from session storage)
  - Implement tool call streaming to frontend
  - Handle agent errors gracefully and send to frontend
  - Add session statistics tracking (per user)
  - Write unit tests (mock Agent, verify streaming)
  - **Acceptance**: Chat messages processed by Yoker Agent; context isolated per user; tool calls streamed to frontend
  - **Priority**: P1 (core backend integration)
  - **Depends on**: 7.4, 7.6, Phase 4 (Agent Runner), 3.3 (Context Management Research)

- [ ] **7.8 Frontend: Authentication UI Flow**
  - Create login page with email input form
  - Create magic link sent confirmation page
  - Create magic link landing page with API key input form
  - Implement API key storage in localStorage after form submission
  - Add logout functionality (clear session, clear localStorage)
  - Add authentication state management (redirect if not authenticated)
  - Handle expired sessions (redirect to login with message)
  - **Acceptance**: Complete auth flow works end-to-end; user redirected appropriately; session state managed correctly
  - **Priority**: P2 (required for production, but can use basic auth for testing)
  - **Depends on**: 7.3, 7.4, 7.5

- [ ] **7.9 Backend: Conversation History Management**
  - Implement conversation list endpoint (GET /api/conversations)
  - Implement conversation load endpoint (GET /api/conversations/:id)
  - Implement conversation delete endpoint (DELETE /api/conversations/:id)
  - Store conversation metadata separately from context
  - Implement conversation search (optional, future enhancement)
  - Add pagination for conversation list
  - Write unit tests for history management
  - **Acceptance**: User can view list of past conversations; load previous conversations; delete conversations
  - **Priority**: P2 (important UX, can defer to Phase 8)
  - **Depends on**: 7.7

- [ ] **7.10 Frontend: Conversation History Sidebar**
  - Create sidebar component for conversation list
  - Implement conversation switching (load different conversation)
  - Add new conversation button
  - Add conversation delete button with confirmation
  - Show current conversation indicator
  - Implement conversation title generation (from first message or LLM summary)
  - **Acceptance**: User can switch between conversations; create new conversations; delete old conversations
  - **Priority**: P2 (important UX, can defer to Phase 8)
  - **Depends on**: 7.9, 7.5

- [ ] **7.11 Production Configuration and Security Hardening**
  - Add HTTPS enforcement (configurable, required for production)
  - Implement secure cookie settings (HttpOnly, Secure, SameSite)
  - Add CSRF protection for non-WebSocket endpoints
  - Add rate limiting for all endpoints
  - Add input sanitization for all user inputs
  - Add security headers (CSP, X-Frame-Options, etc.)
  - Implement environment-based configuration (dev vs production)
  - Add Dockerfile for containerized deployment
  - Add docker-compose.yml with SMTP service
  - **Acceptance**: Security audit passes; deployment guide documented; Docker setup works
  - **Priority**: P2 (required for production deployment)
  - **Depends on**: 7.8, 7.10

- [ ] **7.12 Monitoring and Observability**
  - Add structured logging for webapp (requests, errors, auth events)
  - Add metrics collection (request count, latency, error rate)
  - Add health check endpoint (GET /health with dependency checks)
  - Add WebSocket connection monitoring
  - Add user session tracking (active sessions, login events)
  - Create monitoring dashboard integration (optional, Prometheus/Grafana)
  - **Acceptance**: Logs structured and searchable; health check works; basic metrics collected
  - **Priority**: P3 (important for production, can defer)
  - **Depends on**: 7.11
