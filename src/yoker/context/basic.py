...
class SimpleContextManager(BaseContextManager):
  ...
  def environment_reminder(self) -> str:
    ...
    return (
      ...
      "lines. You MUST use this proactively. Use SPECIFIC patterns — broad terms "
      "like 'error' match test names and produce noise. Examples:\n"
      "  - `post_filter: 'FAILED|Traceback|assert|short test summary'` on make/test "
      "calls to see only failures (not 'error' which matches test names)\n"
