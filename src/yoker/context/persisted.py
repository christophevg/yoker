...
def _is_skill_discovery_block(msg: dict[str, Any]) -> bool:
  """Check whether a message is a framework-generated skill discovery block.

  Skill discovery blocks are injected as ``role="user"`` messages with
  content wrapped in ``<system-reminder>`` tags. They are not real user
  input and should be discarded when resuming a session (a fresh block is
  always generated by ``add_skill_discovery_block``).
  """
...
class Persisted(ContextManagerWrapper):
  ...
  def agent(self, new_agent: "Agent") -> None:
    ...
    if has_loaded_history:
      ...
      # Drop the loaded system messages and old skill-discovery blocks from
      # the head; keep user/assistant/tool messages (the actual conversation).
      # Skill discovery blocks are framework-generated user messages (wrapped
      # in <system-reminder>...</system-reminder>) — not real user input — so
      # they must be filtered out to prevent accumulation across resumes.
      conversation = [
        msg
        for msg in loaded_messages
        if msg.get("role") != "system" and not _is_skill_discovery_block(msg)
      ]
