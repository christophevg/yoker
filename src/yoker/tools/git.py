...
Provides the ``make_git_tool`` factory that returns a callable for
executing Git operations with security guardrails.
"""
...
def make_git_tool(
  ...
  """Create the git tool callable."""
  handlers = permission_handlers or {}
  ...
  async def git(
    ...
    if operation not in config.allowed_commands:
      allowed_list = ", ".join(config.allowed_commands)
      return ToolResult(
        success=False,
        error=f"Operation not allowed: {operation}. Allowed: {allowed_list}",
      )

    allowed, reason = _check_permission(operation, config, handlers)
    if not allowed:
    ...
    try:
      cmd = _build_command(operation, args, config)
...
def _check_permission(
  operation: str,
  config: GitToolConfig,
  permission_handlers: dict[str, HandlerConfig],
) -> tuple[bool, str | None]:
  """Check if operation requires and has permission."""
  if operation not in config.requires_permission:
    return True, None

  handler_key = f"git_{operation}"
  handler = permission_handlers.get(handler_key)
...
__all__ = ["make_git_tool", "OPERATION_ARGS"]
