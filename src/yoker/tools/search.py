...
Provides the ``make_search_tool`` factory that returns a callable for
searching files and their contents. Guardrails are enforced centrally by
the harness based on the schema's ``path`` annotation.
"""
...
def make_search_tool() -> Any:
  """Create the search tool callable."""
  ...
  async def search(
    ...
    try:
      max_results = _clamp(int(max_results), 1, ABSOLUTE_MAX_RESULTS)
      timeout_ms = _clamp(int(timeout_ms), 100, ABSOLUTE_TIMEOUT_MS)
    ...
    try:
      if search_type == "content":
        matches, total, truncated, files_searched = _search_content(
          resolved, search_pattern, max_results, timeout_ms
        )
        result = {
          "success": True,
          "matches": matches,
        ...
        matches, total, truncated = _search_filename(resolved, search_pattern, max_results)
        result = {
          "success": True,
          "matches": matches,
          "total_matches": total,
